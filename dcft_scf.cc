  ...
  DCFTSolver::scf_guess()
  {
      ...
      if (closedshell_){
          ...
          if(guess != "DCFT"){
              ...
              // Check:
//              throw PSIEXCEPTION("SCF_guess is done!");
          }
          else {
  ...
  */
  ...
  DCFTSolver::compute_scf_energy()
  {
      ...
      if(closedshell_){
          ...
          scf_energy_ = enuc_;
//          outfile->Printf( "\t* %d SCF0   Energy = %20.16f\n", macro_cycle, scf_energy_);

          scf_energy_ += kappa_so_a_->vector_dot(so_h_);
//          outfile->Printf( "\t* %d SCF1   Energy = %20.16f\n", macro_cycle, scf_energy_);

          scf_energy_ += kappa_so_a_->vector_dot(Fa_);
//          Fa_->print();
//          outfile->Printf( "\t* %d SCF2   Energy = %20.16f\n", macro_cycle, scf_energy_);

          scf_energy_ += tau_so_a_->vector_dot(so_h_);
//          tau_so_a_->print();
//          outfile->Printf( "\t* %d SCF3   Energy = %20.16f\n", macro_cycle, scf_energy_);

          scf_energy_ += tau_so_a_->vector_dot(Fa_);
//          outfile->Printf( "\t* %d SCF4   Energy = %20.16f\n", macro_cycle, scf_energy_);
...
//                      Db[muNu] = kappa_so_a_->get(h, mu, nu);
                      Ta[muNu] = tau_so_a_->get(h,mu,nu);
                      Tb[muNu] = tau_so_b_->get(h,mu,nu);
//                      Tb[muNu] = tau_so_a_->get(h,mu,nu);
                  }
              }
              soOffset += nsopi_[h];
          }
...
//                        g_tau_b_->set(h, nu, mu, bGTVal);
  ...
  */
  ...
  DCFTSolver::build_AO_tensors()
  {
      ...
      // If the system is closed-shell, then ...
      if(closedshell_){
          dcft_timer_on("DCFTSolver::build_tau(temp) <OO|VV> tensors");
...
//          tau_so_b_->print();
          ...
          // Check:
//          global_dpd_->file2_print(&s_aa_1, "outfile");
//          global_dpd_->file2_print(&s_bb_1, "outfile");
          ...
          do{
              ...
              for(int index = 0; index < iwl->buffer_count(); ++index){
                  ...
                  s = (int) lblptr[labelIndex++];
                  value = (double) valptr[index];
                  AO_contribute(&tau1_AO_ab, &tau2_AO_ab, p, q, r, s, value, &s_aa_1, &s_bb_1, &s_aa_2);
                  ++counter;
          ...
          // Check:
//          global_dpd_->file2_print(&s_aa_2, "outfile");

          // Copy values from GTau (alpha) to GTau (beta)
          global_dpd_->file2_copy(&s_aa_2, PSIF_DCFT_DPD, "s2(temp)B <n|n>");
          ...
          dcft_timer_off("DCFTSolver::build_tau(temp) <OO|VV> tensors");
...
//          global_dpd_->file2_print(&s_aa_2, "outfile");
    ...
    //              outfile->Printf( "\n\t: %5d %5d %5d %10d \n", h, Gc, Gd, offset);
          ...
          global_dpd_->buf4_scm(&tau1_AO_aa, 0.0);
          // Check:
    //      global_dpd_->buf4_print(&tau1_AO_aa, "outfile", 1);
          half_transform(&tau1_AO_aa, &lambda, avir_c_, avir_c_, navirpi_, navirpi_,
                         pq_row_start, CD_row_start, true, 1.0, 0.0);
          global_dpd_->buf4_close(&lambda);
          // Check:
    //      global_dpd_->buf4_print(&tau1_AO_aa, "outfile", 1);
          ...
          // Check:
//          global_dpd_->buf4_print(&tau1_AO_aa, "outfile", 1);
//          global_dpd_->buf4_print(&tau1_AO_bb, "outfile", 1);
//          global_dpd_->buf4_print(&tau1_AO_ab, "outfile", 1);
    ...
    //      global_dpd_->file2_print(&s_bb_1, "outfile");
    //      global_dpd_->file2_print(&s_aa_2, "outfile");
    //      global_dpd_->file2_print(&s_bb_2, "outfile");
          ...
          global_dpd_->buf4_init(&tau2_AO_aa, PSIF_DCFT_DPD, 0, ID("[n,n]"), ID("[O,O]"),
                        ID("[n,n]"), ID("[O,O]"), 0, "tau2AO <nn|OO>");
          // Check:
    //      global_dpd_->buf4_print(&tau2_AO_aa, "outfile", 1);
          global_dpd_->buf4_sort(&tau2_AO_aa, PSIF_DCFT_DPD, rspq, ID("[O,O]"), ID("[n,n]"), "tau2AO <OO|nn>");
          global_dpd_->buf4_close(&tau2_AO_aa);
          global_dpd_->buf4_init(&tau2_AO_aa, PSIF_DCFT_DPD, 0, ID("[O,O]"), ID("[n,n]"),
                        ID("[O,O]"), ID("[n,n]"), 0, "tau2AO <OO|nn>");
