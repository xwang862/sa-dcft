...
//        throw PSIEXCEPTION("TEI in chemists' notation is done!");
        ...
        if(options_.get_str("AO_BASIS") == "NONE") sort_VVVV_integrals();
        ...
        build_denominators();
        // Check:
//        throw PSIEXCEPTION("Building denominators is done!");
...
//        global_dpd_->buf4_sort(&I, PSIF_LIBTRANS_DPD, prqs, ID("[V,O]"), ID("[O,O]"), "MO Ints <VO|OO>");
        global_dpd_->buf4_sort(&I, PSIF_LIBTRANS_DPD, rpsq, ID("[O,V]"), ID("[O,O]"), "MO Ints <OV|OO>");
//        global_dpd_->buf4_sort(&I, PSIF_LIBTRANS_DPD, prqs, ID("[V,o]"), ID("[O,o]"), "MO Ints <Vo|Oo>");
//        global_dpd_->buf4_sort(&I, PSIF_LIBTRANS_DPD, rpsq, ID("[o,V]"), ID("[o,O]"), "MO Ints <oV|oO>");
//        global_dpd_->buf4_sort(&I, PSIF_LIBTRANS_DPD, rpsq, ID("[O,v]"), ID("[O,o]"), "MO Ints <Ov|Oo>");
//        global_dpd_->buf4_sort(&I, PSIF_LIBTRANS_DPD, rpqs, ID("[O,v]"), ID("[o,O]"), "MO Ints <Ov|oO>");
...
//        global_dpd_->buf4_sort(&I, PSIF_LIBTRANS_DPD, prqs, ID("[O,v]"), ID("[V,v]"), "MO Ints <Ov|Vv>");
//        global_dpd_->buf4_sort(&I, PSIF_LIBTRANS_DPD, prsq, ID("[o,V]"), ID("[V,v]"), "MO Ints <oV|Vv>");
        // Intermediate MO_SF <OV|VV> = MO <oV|Vv>
        global_dpd_->buf4_sort(&I, PSIF_LIBTRANS_DPD, prsq, ID("[O,V]"), ID("[V,V]"), "MO Ints SF <OV|VV>");
//        global_dpd_->buf4_sort(&I, PSIF_LIBTRANS_DPD, rpsq, ID("[V,o]"), ID("[V,v]"), "MO Ints <Vo|Vv>");
        global_dpd_->buf4_close(&I);
...
//        global_dpd_->file2_mat_rd(&T_vv);
        ...
        //Beta spin
        *bOccEvals = *aOccEvals;
        bocc_c_->copy(aocc_c_);
        *bVirEvals = *aVirEvals;
        bvir_c_->copy(avir_c_);
